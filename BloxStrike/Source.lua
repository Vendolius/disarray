    ...
    do
        ...
        Visuals.HitSounds = {
            ...
            ["Cod"] = "rbxassetid://5952120301",
            ["Rust"] = "rbxassetid://5043539486",
            ["Minecraft"] = "rbxassetid://4018616850"
        }
        
        Visuals.PlayHitSound = function()
            if not (Flags["visuals hitsound enabled"] and Flags["visuals hitsound enabled"].Value) then
                return
            end
            
            local SoundName = Flags["visuals hitsound"] and Flags["visuals hitsound"].Value or "Neverlose"
            local SoundId = Visuals.HitSounds[SoundName]
            local Volume = Flags["visuals hitsound volume"] and Flags["visuals hitsound volume"].Value or 5
        ...
        Visuals.ApplyWorldVisuals = function()
            ...
            if Flags["visuals world brightness enabled"] and Flags["visuals world brightness enabled"].Value then
                ...
                elseif BrightnessMode == "Dimmed" then
        ...
        Visuals.HookHitSound = function()
            ByteNet.Character.CharacterFlinch.Listen(function()
                Visuals.PlayHitSound()
            end)
        end
        
        Visuals.HookLocalTracers()
        Visuals.HookEnemyTracers()
        Visuals.HookHitSound()
        
        Lighting:GetPropertyChangedSignal("Ambient"):Connect(Visuals.ApplyWorldVisuals)
        Lighting:GetPropertyChangedSignal("OutdoorAmbient"):Connect(Visuals.ApplyWorldVisuals)
        Lighting:GetPropertyChangedSignal("Brightness"):Connect(Visuals.ApplyWorldVisuals)
        Lighting:GetPropertyChangedSignal("ClockTime"):Connect(Visuals.ApplyWorldVisuals)
        ...
        do
            ...
            local TracersSection = VisualsPage:CreateSection({Name = "Bullet Tracers", Side = 2, Length = 0.5}) do
                local LocalTracerToggle = TracersSection:CreateToggle({Name = "Local bullet tracers", Flag = "visuals local tracers"})
                LocalTracerToggle:CreateColorpicker({Color = NewRGB(158, 18, 116), Transparency = 0, Flag = "visuals local tracer color"})
                
                local EnemyTracerToggle = TracersSection:CreateToggle({Name = "Enemy bullet tracers", Flag = "visuals enemy tracers"})
                EnemyTracerToggle:CreateColorpicker({Color = NewRGB(255, 0, 0), Transparency = 0, Flag = "visuals enemy tracer color"})
            end
            
            local HitSoundSection = VisualsPage:CreateSection({Name = "Hit Sound", Side = 2, Length = 0.5}) do
                HitSoundSection:CreateToggle({Name = "Enabled", Flag = "visuals hitsound enabled"})
                HitSoundSection:CreateDropdown({Name = "Sound", Options = {"Neverlose", "Skeet", "Cod", "Rust", "Minecraft"}, Value = "Neverlose", Flag = "visuals hitsound"})
                HitSoundSection:CreateSlider({Name = "Volume", Value = 5, Minimum = 1, Maximum = 10, Flag = "visuals hitsound volume"})
            end
            
            local WorldSection = VisualsPage:CreateSection({Name = "World", Side = 2, Length = 1}) do
                WorldSection:CreateToggle({Name = "Ambient enabled", Flag = "visuals world ambient enabled"})
                local IndoorAmbient = WorldSection:CreateColorpicker({Name = "Indoor ambient", Color = NewRGB(255, 255, 255), Transparency = 0, Flag = "visuals world ambient indoor"})
                local OutdoorAmbient = WorldSection:CreateColorpicker({Name = "Outdoor ambient", Color = NewRGB(255, 255, 255), Transparency = 0, Flag = "visuals world ambient outdoor"})
                
                WorldSection:CreateToggle({Name = "Force time enabled", Flag = "visuals world force time enabled"})
                WorldSection:CreateSlider({Name = "Time of day", Value = 14, Minimum = 0, Maximum = 24, Flag = "visuals world time"})
                
                WorldSection:CreateToggle({Name = "Brightness enabled", Flag = "visuals world brightness enabled"})
                WorldSection:CreateDropdown({Name = "Brightness mode", Options = {"Fullbright", "Dimmed", "Nightmode"}, Value = "Fullbright", Flag = "visuals world brightness"})
            end
        end
