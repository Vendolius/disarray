...
function v_u_54.Enable(p58, p59)
    ...
    if not p59 then
        p58:Cleanup()
        if p58.ancestryChangedConn then
            p58.ancestryChangedConn:Disconnect()
            p58.ancestryChangedConn = nil
        end
    end
end
...
function v_u_54.CharacterAdded(p_u_152, p153, p154)
    if p154 == v_u_1.LocalPlayer then
        ...
        if p_u_152.ancestryChangedConn then
            p_u_152.ancestryChangedConn:Disconnect()
            p_u_152.ancestryChangedConn = nil
        end
        p_u_152.char = p153
...
function v_u_54.Update(p_u_166, _, p167, p168)
    ...
    if not p_u_166.humanoidRootPart then
        ...
        if p_u_166.ancestryChangedConn then
            p_u_166.ancestryChangedConn:Disconnect()
            p_u_166.ancestryChangedConn = nil
        end
        p_u_166.ancestryChangedConn = p_u_166.humanoidRootPart.AncestryChanged:Connect(function(p170, p171)
            if p170 == p_u_166.humanoidRootPart and not p171 then
                p_u_166.humanoidRootPart = nil
                if p_u_166.ancestryChangedConn and p_u_166.ancestryChangedConn.Connected then
                    p_u_166.ancestryChangedConn:Disconnect()
                    p_u_166.ancestryChangedConn = nil
                end
            end
    ...
    local v172 = {}
    p_u_166.behaviorFunction(p_u_166, v172)
    local v173 = { p_u_166.char }
    local v174 = p_u_166.headPart and p_u_166.headPart.CFrame.p or v172[1]
    local v175 = p_u_166.torsoPart and p_u_166.torsoPart.CFrame.p or v172[2]
    local v176 = p_u_166.camera:GetPartsObscuringTarget({ v174, v175 }, v173)
    local v177 = 0
    local v178 = {}
    local v179 = 0.75
    local v180 = {}
    local v181 = 0.75
    for v182 = 1, #v176 do
        local v183 = v176[v182]
        v177 = v177 + 1
        v178[v183] = true
        for _, v184 in pairs(v183:GetChildren()) do
            if v184:IsA("Decal") or v184:IsA("Texture") then
                v177 = v177 + 1
                break
            end
        end
    ...
    if v177 > 0 then
        local v185 = 0.375 / v177 + 0.375
        local v186 = 1 / v177
        v179 = math.pow(v185, v186)
        local v187 = 0.25 / v177 + 0.25
        local v188 = 1 / v177
        v181 = math.pow(v187, v188)
    end
    local v189 = p_u_166.camera:GetPartsObscuringTarget(v172, v173)
    local v190 = {}
    for v191 = 1, #v189 do
        local v192 = v189[v191]
        v190[v192] = v178[v192] and v179 and v179 or v181
        if v192.Transparency < v190[v192] then
            v180[v192] = true
            if not p_u_166.savedHits[v192] then
                p_u_166.savedHits[v192] = v192.LocalTransparencyModifier
            end
        end
        for _, v193 in pairs(v192:GetChildren()) do
            if (v193:IsA("Decal") or v193:IsA("Texture")) and v193.Transparency < v190[v192] then
                v190[v193] = v190[v192]
                v180[v193] = true
    ...
    for v194, v195 in pairs(p_u_166.savedHits) do
        if v180[v194] then
            v194.LocalTransparencyModifier = v194.Transparency < 1 and ((v190[v194] - v194.Transparency) / (1 - v194.Transparency) or 0) or 0
        else
            v194.LocalTransparencyModifier = v195
            p_u_166.savedHits[v194] = nil
        end
    end
    return p167, p168
end
