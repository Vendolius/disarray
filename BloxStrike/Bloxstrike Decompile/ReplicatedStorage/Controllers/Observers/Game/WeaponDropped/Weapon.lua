...
function v_u_1.updateState(p38, p39)
    ...
    if p38.Model.PrimaryPart then
        ...
        if v43 == "Hovering" or v43 == "Proximity" then
            ...
            if v43 == "Proximity" and v_u_6.Get(v_u_16, "Settings.Game.Item.Auto Pickup Dropped Weapons") ~= false then
                p38:autoPickup()
                return
            end
        else
...
function v_u_1.new(p46)
    local v47 = v_u_1
    local v_u_48 = setmetatable({}, v47)
    v_u_48.Janitor = v_u_9.new()
    v_u_48.Model = p46
    v_u_48.AlphaTime = 0
    v_u_48.Weapon = v_u_48.Model:GetAttribute("Weapon")
    v_u_48.Skin = v_u_48.Model:GetAttribute("Skin")
    v_u_48.SentPickupRequest = false
    v_u_48.Janitor:Add(v_u_3.Heartbeat:Connect(function(p49)
        if v_u_48.Model and v_u_48.Model.PrimaryPart then
            v_u_48:updateState(p49)
        end
    end))
    return v_u_48
end
function v_u_1.destroy(p50)
    p50.Janitor:Destroy()
end
return v_u_1